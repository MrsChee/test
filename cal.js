...
function DrawCal(ShowedYear, ShowedMonth, currentMoment, cal) {
  // отрисовка месяца
...
function getPrewYear(year, month) {
  //предыдущий год
  if (month == 0) {
    return year - 1
  } else {
...
function showInfo(year, month, elem) {
  //инфа о месяце и годе сверху
  elem.innerHTML = getMonthName(month) + " " + year
}
function getMonthName(num) {
  //номер месяца в слово
  var months = [
    "Январь",
    "Февраль",
    "Март",
...
function createtable(arr, parent) {
  //создает двумерный массив цифр в календаре
  parent.innerHTML = ""
...
function createArr(from, to) {
  //создает двумерный массив в пределе заданых чисел
  var arr = []
...
function getFirstDayNum(year, month) {
  //номер первого дня месяца
...
function nedelyaArr(num, arr) {
  //разбиваем месяц на недели num=7
  var a = []
